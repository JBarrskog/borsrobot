...
class TestMovingAverage(unittest.TestCase):
    ...
    def test_algorithm(self):
        ...
        np.testing.assert_array_equal(
            moving_average.simple_moving_average(self.data_frame, 'col2', 3)[
                'SMA_3'].values, answer)
    ...
    def test_name_of_new_column(self):
        ...
        for window in range(1, 9):
            self.data_frame = moving_average.simple_moving_average(
                self.data_frame, 'col2', window)

            answer.append('SMA_{}'.format(window))

        self.assertEqual(list(self.data_frame), answer)

        # Testing exceptions
        for tst in [True, [1, 2, 3], 2, 2.0]:
            self.assertRaises(TypeError, moving_average.simple_moving_average,
                              self.data_frame, tst, 3)
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, "some sting", 3)

    def test_window(self):
        """Testing different kinds of windows."""
        # Different integers
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', len(self.data_frame)+1)
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', len(self.data_frame)+10000)
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', 0)
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', -1)
        self.assertRaises(ValueError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', -10000)

        # Float, string, bool, complex, list
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', 3.)
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', "three")
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', True)
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', 3+5j)
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          self.data_frame, 'col2', [1, 2, 3])
    ...
    def test_data_frame(self):
        """Testing different kinds of self.data_frame."""
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          np.arange(10), 'col2', 3)
        self.assertRaises(TypeError, moving_average.simple_moving_average,
                          1, 'col2', 3)
